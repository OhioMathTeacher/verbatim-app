...
const Calculator = (() => {
  ...
  let pane = "calc", win = null, cv = null, ctx = null, target = null;
  ...
  const KEYS = [
    ...
    ["π", "π", "fn"], ["x", "x", "fn"], ["^", "^", "fn"], ["!", "!", "fn"], ["@clear", "C", "del"],
    ["7", "7", ""], ["8", "8", ""], ["9", "9", ""], ["(", "(", "op"], [")", ")", "op"],
    ["4", "4", ""], ["5", "5", ""], ["6", "6", ""], ["×", "×", "op"], ["÷", "÷", "op"],
    ["1", "1", ""], ["2", "2", ""], ["3", "3", ""], ["+", "+", "op"], ["−", "−", "op"],
  ...
  function build(){
    host.innerHTML = `
      ...
      <div class="cal-body">
        <div class="cal-pane on" data-pane="calc">
          <div class="cal-entry"><input id="cal-in" inputmode="text" autocomplete="off" spellcheck="false" placeholder="0.2 - 0.2^3/3!"></div>
          <div class="cal-out" id="cal-out">&nbsp;</div>
        </div>
        <div class="cal-pane" data-pane="graph">
          <div class="cal-entry"><label>y =</label><input id="cal-gin" autocomplete="off" spellcheck="false" placeholder="sin(x)"></div>
        ...
        <div class="cal-pane" data-pane="table">
          <div class="cal-entry"><label>f(x) =</label><input id="cal-tin" autocomplete="off" spellcheck="false" placeholder="sin(x) - x"></div>
          <div class="cal-two">
    ...
    $("#cal-tabs").onclick = e => {
      const b = e.target.closest("button"); if(!b) return;
      pane = b.dataset.pane;
      [...$("#cal-tabs").children].forEach(x => x.classList.toggle("on", x === b));
      host.querySelectorAll(".cal-pane").forEach(p => p.classList.toggle("on", p.dataset.pane === pane));
      target = host.querySelector(`.cal-pane[data-pane="${pane}"] input`);
      if(pane === "graph") graph();
      if(pane === "table") table();
    };
    host.querySelectorAll(".cal-pane input").forEach(i => {
      i.addEventListener("focus", e => target = e.target);
      i.addEventListener("input", () => refresh(false));
      i.addEventListener("keydown", e => { if(e.key === "Enter"){ e.preventDefault(); refresh(true); } });
    });
    $("#cal-pad").onclick = e => {
      const b = e.target.closest("button"); if(!b) return;
      const t = target && t_ok(target) ? target : host.querySelector(`.cal-pane[data-pane="${pane}"] input`);
      const k = b.dataset.k;
      if(k === "@clear") t.value = "";
      else if(k === "@del"){
        const p = t.selectionStart;
        if(p > 0){ t.value = t.value.slice(0, p - 1) + t.value.slice(t.selectionEnd); t.setSelectionRange(p - 1, p - 1); }
      } else if(k === "@go"){ refresh(true); t.focus(); return; }
      else {
  ...
  const t_ok = t => t && t.isConnected;

  /* Drawn from the page's own colours, read at paint time, so the drawer
  ...
  function graph(){
    ...
    ctx.lineWidth = 1; ctx.strokeStyle = ink("--rule");
    ctx.beginPath();
    ...
    ctx.strokeStyle = ink("--ai") || "#2f5d6b"; ctx.lineWidth = 1.8;
    ctx.beginPath();
  ...
  function table(){
    ...
    catch(e){ h.innerHTML = `<p class="cal-err">${e.message}</p>`; return; }
    let rows = "";
    for(let i = 0; i < 20; i++){
  ...
  function calc(record){
    const src = $("#cal-in").value.trim(), out = $("#cal-out");
    out.classList.remove("err");
    if(!src){ out.innerHTML = "&nbsp;"; return; }
    try {
      const v = Calc.eval(src, {}, deg);
      out.textContent = show(v);
      if(record) onUse({ kind: "evaluate", expr: src, result: v, deg });
    } catch(err){ out.classList.add("err"); out.textContent = err.message; }
  }

  function refresh(record){
    if(pane === "calc") calc(record);
    if(pane === "graph"){
      graph();
      const e = $("#cal-gin").value.trim();
      if(record && e) onUse({ kind: "graph", expr: e, deg });
    }
    if(pane === "table"){
      table();
      const e = $("#cal-tin").value.trim();
      if(record && e) onUse({ kind: "table", expr: e, deg,
  ...
  const api = {
    ...
    open(){
      ...
      const t = host.querySelector(`.cal-pane[data-pane="${pane}"] input`);
