...
<div class="ai-modal-overlay" id="ai-modal-overlay">
  <div class="ai-modal" role="dialog" aria-labelledby="ai-modal-title" aria-modal="true">
    ...
    <div class="ai-tabs" role="tablist">
      <button class="ai-tab" role="tab" data-tab="local" aria-selected="false" data-i18n="tablocal">Local / personal</button>
    ...
    <div id="ai-local-help" class="localpanel hide"></div>
...
const I18N = {
  en: {
    ...
    aihint: "Use your own API key, or the one your instructor gives you. It stays in this browser tab and is gone when you close it. Whichever AI you use is recorded with your session.",
    ...
    aisetup: "AI Setup",
    modalsub: "Choose where your AI teacher runs, then enter a key if it needs one. Nothing is stored beyond this browser tab.",
    tablocal: "Local / personal", tabfree: "Free ($0)", tabpaid: "Paid",
    notefree: "Nothing here is available on this network. Two things still work: a model running on your own computer (Local / personal), or the key your instructor gives you, used with one of the services under Paid.",
    ...
    forgetconfirm: "Forget every API key stored in this browser tab?",
    ...
    localh: "Running a model on your own computer",
    ...
    keyhint2: ". It stays in this tab only.",
  ...
  zh: {
    ...
    aihint: "可以使用你自己的 API 密钥，或老师提供的密钥。密钥只保存在本标签页中，关闭后即清除。你使用的 AI 会随会话一起记录。",
    start: "开始活动",
    ...
    aisetup: "AI 设置",
    modalsub: "选择你的 AI 老师在哪里运行，如需密钥请填写。所有内容仅保存在本标签页中。",
    tablocal: "本地 / 个人", tabfree: "免费 ($0)", tabpaid: "付费",
    notefree: "\u5728\u5f53\u524d\u7f51\u7edc\u4e0b\uff0c\u8fd9\u91cc\u6ca1\u6709\u53ef\u7528\u7684\u670d\u52a1\u3002\u4ecd\u6709\u4e24\u4e2a\u53ef\u884c\u7684\u529e\u6cd5\uff1a\u5728\u81ea\u5df1\u7684\u7535\u8111\u4e0a\u8fd0\u884c\u6a21\u578b\uff08\u201c\u672c\u5730 / \u4e2a\u4eba\u201d\uff09\uff0c\u6216\u4f7f\u7528\u8001\u5e08\u63d0\u4f9b\u7684\u5bc6\u94a5\u8bbf\u95ee\u201c\u4ed8\u8d39\u201d\u4e2d\u7684\u670d\u52a1\u3002",
    ...
    forgetkeys: "清除所有密钥",
    forgetconfirm: "确定要清除本标签页中保存的所有 API 密钥吗？",
...
/* Where to get a model if you do not have one. ModelScope is the domestic hub and
   hf-mirror the usual mirror of HuggingFace. All three are always listed, so no
   student is told which side of anything they are on; only the order changes, and
   it follows the language toggle -- a stated reading preference, which is the one
   signal here that costs the student nothing to give. */
const LOCAL_STEPS = [
  { en: ["Install a model server", "Ollama or LM Studio. This is the program that runs the model and lets this page talk to it."],
    zh: ["安装一个模型服务", "Ollama 或 LM Studio。它负责运行模型，并让本页面可以访问。"],
    links: [["Ollama", "https://ollama.com/download"], ["LM Studio", "https://lmstudio.ai"]] },
  { en: ["Download a model", "In Ollama: ollama pull qwen2.5 — or get model files from a hub."],
    zh: ["下载一个模型", "在 Ollama 中执行：ollama pull qwen2.5 —— 或从模型社区下载模型文件。"],
    // Ordered by the language the student chose. Nothing is detected and nothing
    // is asked: the toggle in the masthead is a stated reading preference, and it
    // is the only signal here that costs the student nothing to give.
    links_en: [["HuggingFace", "https://huggingface.co/models"],
               ["ModelScope \u9b54\u642d", "https://modelscope.cn/models"],
               ["hf-mirror", "https://hf-mirror.com"]],
    links_zh: [["ModelScope \u9b54\u642d", "https://modelscope.cn/models"],
               ["hf-mirror", "https://hf-mirror.com"],
               ["HuggingFace", "https://huggingface.co/models"]],
    note: { en: "HuggingFace may not load from mainland China. ModelScope is the domestic hub and hf-mirror mirrors HuggingFace \u2014 both work either way.",
            zh: "HuggingFace \u5728\u4e2d\u56fd\u5927\u9646\u53ef\u80fd\u65e0\u6cd5\u6253\u5f00\u3002ModelScope \u662f\u56fd\u5185\u7684\u6a21\u578b\u793e\u533a\uff0chf-mirror \u662f HuggingFace \u7684\u955c\u50cf \u2014\u2014 \u4e24\u8005\u5728\u4efb\u4f55\u60c5\u51b5\u4e0b\u90fd\u53ef\u7528\u3002" } },
  { en: ["Let this page reach it", "Ollama refuses browsers by default. Start it with:"],
    zh: ["允许本页面访问", "Ollama 默认拒绝浏览器访问。请这样启动："],
    code: "OLLAMA_ORIGINS=* ollama serve" },
  { en: ["Add it above", "Click \u201cAdd a server\u201d and enter the address:"],
    zh: ["在上方添加", "点击\u201c添加服务器\u201d并填写地址："],
    code: "http://localhost:11434" }
];
function renderLocalHelp(){
  const el = $("#ai-local-help");
  el.classList.toggle("hide", aiTab !== "local");
  if(aiTab !== "local") return;
  el.textContent = "";
  const h = document.createElement("h3"); h.textContent = tr("localh"); el.appendChild(h);
  const grid = document.createElement("div"); grid.className = "steps"; el.appendChild(grid);
  LOCAL_STEPS.forEach((st, i) => {
...
function renderNotes(){
  ...
  if(aiTab !== "local" && !offer(aiTab).length) add("notefree");
}
...
$("#gear").onclick = $("#ai-open").onclick = () => {
  ...
  const c = aiConfig();
  if(c && !c.local){ const p = PROVIDERS.find(x => x.id === c.id); if(p){ aiTab = p.tier; showKeyEntry(p); } }
  for(const t of document.querySelectorAll(".ai-tab")) t.setAttribute("aria-selected", String(t.dataset.tab === aiTab));
  renderCards();
  if(aiTab === "local") probeWellKnown();
};
...
function validateSurvey(v){
  ...
  for(const q of v){
    ...
    if(!["single", "multi", "text"].includes(q.type)) return tr("toolerrtype") + q.type;
